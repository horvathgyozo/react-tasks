...
2. Szeretnénk a menüt működőképessé tenni, és az egyes menüpontokra kattintva a megfelelő komponenseket megjeleníteni, azaz az oldalon belül navigálni.
   1. Telepítsd fel ehhez a `react-router` csomagot. [Használd a dokumentációt is!](https://reactrouter.com/start/declarative/installation)
   2. Segítségével oldd meg, hogy az egyes végpontokon a megfelelő komponensek jelenjenek meg. Ehhez a `BrowserRouter`, a `Routes` és a `Route` komponenseket kell használnod. A megoldást rakd az `App` komponensbe. ([Dokumentáció](https://reactrouter.com/start/declarative/routing#configuring-routes))
   3. Tedd működőképessé a menüt! Cseréld ki a linkeket `Link` vagy sokkal inkább `NavLink` komponensekre! ([Dokumentáció](https://reactrouter.com/start/declarative/navigating))
   4. Próbáld meg a `Layout` komponenst legfelsőbb szintű route-ként használni, és benne szereplő route-okkal megjeleníteni az oldalakat. Ehhez az `Outlet` komponensre is szükséged lesz a `Layout`-on belül. Oldd meg, hogy a `<Home>` komponens legyen az alapértelmezett (`index` prop) és ismeretlen végpontnál irányítson erre az oldalra (`<Navigate>` komponens)! (A dokumentációban szükséges lesz a [Nested Routes](https://reactrouter.com/start/declarative/routing#nested-routes) és a [Layout Routes](https://reactrouter.com/start/declarative/routing#layout-routes) részek elolvasása is!)
